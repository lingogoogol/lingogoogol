...
import external.ShObjIdl;
import external.objbase;
import external.processenv;
import external.shellapi;

import lgo.container;
import lgo.io;
import lgo.GUI;
...
public:
	...
	auto OnFileOk(IFileDialog* in) -> HRESULT override
	{
		...
		in->QueryInterface(IID_PPV_ARGS0(&dialog), IID_PPV_ARGS1(&dialog));
		...
		for (DWORD i{ 0 }; i < result_count; ++i)
		{
			...
			m_result[i] = lgo::to_string(std::wstring{ result_strc });
		}
		m_ok = true;
		return S_OK;
	}
...
struct loading_folder_t
{
	...
	std::shared_ptr<lgo::text_line_scroll_std_t> m_progress_bar{};
};
...
struct loading_list_t
{
	...
	std::shared_ptr<lgo::divy_soft_t> m_progress_bar_div{};
	std::mutex m_mutex{};
};
*/
auto message_loop
(
	lgo::engine_t* engine/*,
	loading_list_t* uploading_list,
	loading_list_t* downloading_list*/
)
-> bool
{
...
-> std::vector<std::string>
{
	...
	(
		...
		IID_PPV_ARGS0(&dialog), 
		IID_PPV_ARGS1(&dialog)
	);
...
auto to_date_time(FILETIME file) -> std::string
{
	...
	return
		std::to_string(system.wYear) + "-" +
		std::to_string(system.wMonth) + "-" +
		std::to_string(system.wDay) +
		"T" + std::to_string(system.wHour) + ":" +
		std::to_string(system.wMinute) + ":" +
		std::to_string(system.wSecond) + "Z";
}
...
auto data_upload
(
	curl_multipart_t& multipart,
	blob_empty_queue_t& empty_blob,
	std::string name,
	std::string content
)
-> std::string
...
auto folder_create_callback(lgo::engine_t* engine, lgo::state_t* state, std::string* pos) -> void
{
	return;
}

auto folder_data_add
(
	blob_empty_queue_t& empty_blob,
...
(
	...
	blob_empty_queue_t& empty_blob,
	loading_t* loading,
	std::shared_ptr<loading_folder_t> loading_folder,
	std::shared_ptr<lgo::divy_soft_t> progress_bar_div,
	std::string pos,
	std::filesystem::path file,
	std::deque<loading_file_t>* list,
	std::mutex& mutex
)
-> std::string
{
	...
	if (progress_bar_div)
	{
		...
		uploading.m_progress_bar =
			progress_bar_div->add_object_shared<lgo::text_line_scroll_std_t>
			(
				lgo::depth_range_t{ 0.0f, 0.0f },
				"檔案：0B/?B " + file.string(),
				lgo::std_blue_light,
				lgo::size_1D{ 0x100 }
			);
...
(
	...
	std::deque<loading_file_t>* uploading_list,
	std::mutex& mutex
)
-> std::string
{
	...
	http_api_request_action(multipart);
	lgo::log_file(folder.string() + "\n");
	return main.m_id;
}

auto folder_upload_callback
(
	lgo::engine_t* engine,
	std::shared_ptr<lgo::divy_soft_t> progress_bar_div,
	std::string* pos,
...
-> void
{
	...
	for (auto i : folder)
	{
		...
		loading->m_name = i;
		loading->m_pos = *pos;
		loading->m_progress_bar_id =
			progress_bar_div->add_object<lgo::divy_flex_soft_t>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				progress_bar_div->space_x(lgo::std_margin),
				lgo::std_margin,
				lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::bottom },
				true
			);
...
-> void
{
	...
	for (auto i : file)
	{
		...
		loading->m_progress_bar_id =
			...
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				progress_bar_div->space_x(std_margin),
				std_margin,
				lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::bottom },
				true
			);
...
(
	lgo::engine_t* engine,
	std::shared_ptr<lgo::divy_soft_t> download_progress_bar_div,
	std::shared_ptr<lgo::divy_soft_t> viewer_div,
	std::string* pos,
	loading_list_t* uploading_list,
	loading_list_t* downloading_list
)
...
auto delete_all_confirm_callback
(
	std::shared_ptr<lgo::divy_hard_t> center_div,
	std::shared_ptr<lgo::divy_soft_t> progress_bar_div,
	std::shared_ptr<lgo::text_line_flex_std_input_t> input,
	loading_list_t* uploading_list
)
{
...
auto delete_all_cancel_callback(std::shared_ptr<lgo::divy_hard_t> center_div)
{
	center_div->clear_state();
	return;
}

auto delete_all_callback
(
	std::shared_ptr<lgo::divy_hard_t> center_div,
	std::shared_ptr<lgo::divy_soft_t> progress_bar_div,
	loading_list_t* uploading_list
...
-> void
{
	auto input_div
	{
		center_div->add_object_shared<lgo::divy_flex_soft_t>
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			lgo::size_1D{ 0x400 },
			lgo::std_margin,
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::center }
		)
	};
	input_div->add_object<lgo::text_line_flex_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.0f },
		"真的要清除的話輸入\"清除\"",
		lgo::std_sideways,
	...
	auto focus{ new lgo::focus_t{} };
	auto input
	{
		input_div->add_object_shared<lgo::text_line_flex_std_input_t>
		(
			lgo::depth_range_t{ 0.0f, 0.0f },
			"",
			focus,
	...
	input_div->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&delete_all_confirm_callback, center_div, progress_bar_div, input, uploading_list),
		"確認"
	);
	input_div->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&delete_all_cancel_callback, center_div),
		"取消"
	);
	return;
}

auto folder_button_callback
(
	lgo::engine_t* engine,
	std::shared_ptr<lgo::divy_soft_t> download_progress_bar_div,
	std::shared_ptr<lgo::divy_soft_t> viewer_div,
...
-> void
{
	...
	for (auto i : obj_list)
	{
		if (i["type"] == "folder")
		{
			viewer_div->add_object<button_text_t<lgo::text_line_scroll_std_t>>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				std::bind
				(
					&folder_button_callback,
					engine,
					download_progress_bar_div,
					viewer_div,
					pos,
				...
				"資料夾，名稱：" + i["name"].get<std::string>(),
				std_white,
				size_1D{ viewer_div->space_2D(std_margin).x }
			);
			viewer_div->add_object<lgo::button_text_t<lgo::text_line_std_t>>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				std::bind
				(
					&folder_download_callback,
					engine,
					download_progress_bar_div,
				...
				"下載",
				lgo::std_white,
				viewer_div->space_x(std_margin)
			);
			viewer_div->add_object<lgo::std_linex_t>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				viewer_div->space_x(std_margin),
				lgo::std_white
		...
		{
			viewer_div->add_object<lgo::button_text_t<lgo::text_line_scroll_std_t>>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				[] () {},
				"檔案，名稱：" + i["name"].get<std::string>() + "，大小：" + std::to_string(i["size"].get<std::uint64_t>()),
				lgo::std_white,
				lgo::size_1D{ viewer_div->space_2D(std_margin).x }
			);
			viewer_div->add_object<lgo::button_text_t<lgo::text_line_std_t>>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
		...
		{
			throw lgo::internal_error_t{ "obj type" };
		}
	}
	return;
}
...
(
	lgo::engine_t* engine,
	lgo::state_t* state,
	std::shared_ptr<lgo::divy_soft_t>* upload_progress_bar_div,
	std::shared_ptr<lgo::divy_soft_t>* download_progress_bar_div,
	std::string* pos,
	loading_list_t* uploading_list,
	loading_list_t* downloading_list
...
-> void
{
    ...
    auto main_div
	{
		state->add_object_shared<lgo::divy_hard_t>
		(
			lgo::depth_range_t{ 0.0f, 1.0f },
			pos_2D{ 0, 0 },
			engine->get_window_size(),
			lgo::size_1D{ 0 },
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::top }
		)
	};

    auto menu_left
	{
		main_div->add_object_ghost_shared<lgo::divx_soft_t>
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			lgo::div_create_from_member{},
			lgo::std_forward,
			lgo::std_margin,
			main_div->space_x(lgo::std_margin),
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::center }
		)
	};
    menu_left->add_object<lgo::text_line_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		"雲端硬碟",
		lgo::std_white,
    ...
    auto menu_right
	{
		main_div->add_object_shared<lgo::divx_soft_t>
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			lgo::div_create_from_member{},
			lgo::std_forward,
			lgo::std_margin,
			main_div->space_x(lgo::std_margin),
			lgo::alignment_2D{ lgo::alignment_x::right, lgo::alignment_y::center }
		)
	};
    main_div->add_object_shared<lgo::std_linex_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		main_div->space_x(lgo::std_margin),
		lgo::std_white
    ...
    auto work_div
	{
		...
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			main_div->space_2D(lgo::std_margin),
			lgo::std_margin,
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::top }
		)
	};
    ...
    auto path_div
	{
		work_div->add_object_ghost_shared<lgo::divy_soft_t>
		(
			depth_range_t{ 0.0f, 0.5f },
			lgo::size_2D{ 0x100, work_div->space_y(lgo::std_margin).x },
			lgo::std_margin,
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::top }
		)
	};
	*upload_progress_bar_div =
		...
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			lgo::size_2D{ 0x100, work_div->space_y(std_margin).x },
			lgo::std_margin,
			lgo::alignment_2D{ lgo::alignment_x::left, lgo::alignment_y::bottom },
			true
		);
	*download_progress_bar_div =
		work_div->add_object_shared<lgo::divy_soft_t>
		(
			lgo::depth_range_t{ 0.0f, 0.5f },
			lgo::size_2D{ 0x100, work_div->space_y(lgo::std_margin).x },
			lgo::std_margin,
    ...
    work_div->add_object<lgo::std_liney_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		work_div->space_y(lgo::std_margin),
		lgo::std_white
	);
    auto viewer_div
	{
	...
	auto center_div
	{
		...
		(
			...
			engine->get_window_size(),
			size_1D{ 0 },
			lgo::alignment_2D{ lgo::alignment_x::center, lgo::alignment_y::center }
		)
	};
	
    menu_right->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&folder_create_callback, engine, state, pos),
		"新增資料夾"
	);
    menu_right->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&folder_upload_callback, engine, *upload_progress_bar_div, pos, uploading_list, downloading_list),
		"上傳資料夾"
	);
    menu_right->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&file_upload_callback, engine, *upload_progress_bar_div, pos, uploading_list, downloading_list),
		"上傳檔案"
	);
    menu_right->add_object<lgo::button_classic_std_t>
	(
		lgo::depth_range_t{ 0.0f, 0.5f },
		std::bind(&delete_all_callback, center_div, *upload_progress_bar_div, uploading_list),
		"清除"
	);
	list
	(
...
auto upload(loading_list_t* list, blob_empty_queue_t* empty_blob) -> void
{
	...
	while (!close_app)
	{
		...
		if (!list->m_list_process.empty())
		{
			...
			progress_bar_div->add_object<lgo::std_linex_t>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				progress_bar_div->space_x(lgo::std_margin),
				lgo::std_white
		...
		if (!file_list.empty())
		{
			...
			if (uploading.m_progress < uploading.m_size)
			{
				...
				{
					std::min(uploading.m_size - uploading.m_progress, content_size_max_base64 * (batch_request_max - 3))
				};
				...
				std::uint64_t id_i{ 0 };
				curl_multipart_t multipart{ http_api_multipart(curl_upload(), account_main) };
				std::ifstream content_stream{ lgo::create_ifstream(uploading.m_name) };
				...
				if (full)
				{
					data_name_set_request
					(
						multipart,
						uploading.m_main_last_id,
						nlohmann::json
						{
				...
				{
					uploading.m_progress_bar->set_text
					(
						"檔案：" + std::to_string(uploading.m_progress) + "B/" +
						std::to_string(uploading.m_size) + "B " +
						uploading.m_name
					);
				}
				if (uploading.m_folder)
				{
					...
					uploading.m_folder->m_progress_bar->set_text
					(
						"資料夾：" + std::to_string(uploading.m_folder->m_progress) + "B/" +
						std::to_string(uploading.m_folder->m_size) + "B " +
						uploading.m_folder->m_name
					);
				}
		...
		else if (!delete_id.empty())
		{
			...
			for (std::uint64_t i{ 0 }; i < delete_count; ++i)
			{
				...
				delete_id[i].m_progress_bar->set_text
				(
					"刪除：" +
					std::to_string(delete_id[i].m_loading->m_progress) + "/" +
					std::to_string(delete_id[i].m_loading->m_size) + " " +
					delete_id[i].m_loading->m_name
				);
			}
...
auto download(loading_list_t* list) -> void
{
	...
	while (!close_app)
	{
		...
		{
			auto obj_ptr{ list->m_list_process.begin()->get() };
			auto progress_bar_div{ list->m_progress_bar_div->get_object_shared<lgo::divy_flex_soft_t>(obj_ptr->m_progress_bar_id) };
			progress_bar_div->add_object<lgo::std_linex_t>
			(
				lgo::depth_range_t{ 0.0f, 0.5f },
				progress_bar_div->space_x(std_margin),
		...
		if (!file_list.empty())
		{
			...
			if (downloading.m_progress < downloading.m_size)
			{
				...
				std::ofstream content_stream{ lgo::create_ofstream_preserve(downloading.m_name) };
				...
				{
					downloading.m_progress_bar->set_text
					(
						"檔案：" +
						std::to_string(downloading.m_progress) + "B/" +
						std::to_string(downloading.m_size) + "B " +
						downloading.m_name
					);
				}
				if (downloading.m_folder)
				{
					...
					downloading.m_folder->m_progress_bar->set_text
					(
						"資料夾：" +
						std::to_string(downloading.m_folder->m_progress) + "B/" +
						std::to_string(downloading.m_folder->m_size) + "B " +
						downloading.m_folder->m_name
					);
				}
...
auto wWinMain(HINSTANCE, HINSTANCE, PWSTR, int) -> int
{
	lgo::init_logfile("logfile" + std::to_string(std::time(nullptr)) + ".txt");
    lgo::log_file("CWD:\n");
	lgo::log_file("  " + std::filesystem::current_path().string() + "\n\n");
    /*init_directx();
	freetype_t freetype{};
    ...
    lgo::engine_t engine{ lgo::size_2D{ 0x600, 0x400 }, "wWinMain.engine" };
