...
auto create_factory() -> Microsoft::WRL::ComPtr<IDXGIFactory5> {
    Microsoft::WRL::ComPtr<IDXGIFactory5> out{};
    CreateDXGIFactory2(DXGI_CREATE_FACTORY_DEBUG, IID_PPV_ARGS(&out));
    return out;
}

auto create_window_class(const std::wstring& name, WNDPROC windows_process
, HINSTANCE instance, HICON icon, HICON small_icon, HCURSOR cursor) -> ATOM {
...
auto create_window(ATOM window_class, const std::wstring& name, DWORD style
...
-> Microsoft::WRL::ComPtr<IDXGIAdapter4> {
    ...
    for (UINT i{ 0 }; true; ++i) {
        ...
        && SUCCEEDED(D3D12CreateDevice(out.Get(), D3D_FEATURE_LEVEL_12_0
...
auto create_device(Microsoft::WRL::ComPtr<IDXGIAdapter4> adapter)
...
auto create_info_queue(Microsoft::WRL::ComPtr<ID3D12Device2> device)
...
auto create_V_heap(Microsoft::WRL::ComPtr<ID3D12Device2> device, D3D12_DESCRIPTOR_HEAP_TYPE type
, UINT count) -> Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> {
...
auto create_RT(Microsoft::WRL::ComPtr<ID3D12Device2> device
, Microsoft::WRL::ComPtr<IDXGISwapChain4> swap_chain
, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> RTV_heap, UINT RTV_size, UINT count)
